...
#include "SolanaMainnetProvider.h"
...
SolanaMainnetProvider::SolanaMainnetProvider()
{
}

QCoro::Task<std::optional<Monitor::BalanceResult>>
SolanaMainnetProvider::fetchBalance(QNetworkAccessManager* networkManager,
    const QString& address, ChainType chain)
{
    ...
    try {
        ...
        spdlog::error("Exception in Solana Mainnet fetch for {}: {}",
...
QString SolanaMainnetProvider::getApiEndpoint() const
{
    return apiEndpoint_;
}
}
