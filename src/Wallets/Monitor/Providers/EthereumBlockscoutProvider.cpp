...
#include "EthereumBlockscoutProvider.h"
...
EthereumBlockscoutProvider::EthereumBlockscoutProvider()
{
}

QCoro::Task<std::optional<Monitor::BalanceResult>>
EthereumBlockscoutProvider::fetchBalance(QNetworkAccessManager* networkManager,
    const QString& address, ChainType chain)
{
    ...
    try {
        ...
        try {
            result.balance = j["result"].get<double>() / 1e18; // Wei to ETH
            result.success = true;
        ...
        result.errorMessage = QString::fromStdString(e.what());
        spdlog::error("Exception in Ethereum Blockscout fetch for {}: {}",
...
QString EthereumBlockscoutProvider::getApiEndpoint() const
{
    return apiEndpoint_;
}
}
