...
#include "BitcoinBlockchainProvider.h"
...
BitcoinBlockchainProvider::BitcoinBlockchainProvider()
{
}

QCoro::Task<std::optional<Monitor::BalanceResult>>
BitcoinBlockchainProvider::fetchBalance(QNetworkAccessManager* networkManager,
    const QString& address, ChainType chain)
{
    ...
    try {
        ...
        try {
            result.balance = j["final_balance"].get<double>() / 1e8;
            result.success = true;
        ...
        spdlog::error("Exception in Bitcoin Blockchain fetch for {}: {}",
...
QString BitcoinBlockchainProvider::getApiEndpoint() const
{
    return apiEndpoint_;
}
}
